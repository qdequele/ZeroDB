...
fn to_oracle(e: heed::Error) -> OracleError {
    match e {
        heed::Error::Mdb(m) => match m {
            ...
            // Defensive: no probed stale-dbi path returns MDB_BAD_DBI (the
            // fork uses raw EINVAL, below), but if one ever does it is the
            // same normalized state (see `OracleError::BadDbi`).
            heed::MdbError::BadDbi => OracleError::BadDbi,
            other => OracleError::Other(format!("mdb:{other:?}")),
        },
        // Raw `EINVAL` from the `TXN_DBI_EXIST` stale-dbi gate (M2.9; probed
        // 2026-07-22 — the only InvalidInput producer in the modeled surface).
        heed::Error::Io(io) if io.kind() == std::io::ErrorKind::InvalidInput => OracleError::BadDbi,
...
impl LmdbEngine {
    ...
    fn drop_db(&mut self, db: u8) -> OpResult {
        ...
        // deliberate stale-handle probe (M2.9), which is memory-safe: a
        // closed dbi fails LMDB's `TXN_DBI_EXIST` gate with `EINVAL` before
        // any dereference (probed 2026-07-22, every op class incl. a second
        // `remove`).
        ...
        match res {
            Ok(()) => {
                ...
                // later abort. The entry is KEPT, dead (M2.9): later uses are
                // differential stale-handle probes.
