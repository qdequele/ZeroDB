...
impl RedbDb {
    fn new(path: &std::path::Path) -> anyhow::Result<Self> {
        ...
        {
            let _table =
                write_txn.open_table::<&[u8], &[u8]>(redb::TableDefinition::new("bench"))?;
        }
        write_txn.commit()?;
                ...
                b.iter_batched(
                    ...
                    |(db, _temp_dir)| {
                        ...
                        match db.write_batch(black_box(data)) {
                            ...
                            Err(e) => {
                                eprintln!(
                                    "Write batch failed for {} with size {}: {:?}",
                                    db_name, size, e
                                );
...
fn bench_random_reads(c: &mut Criterion) {
    ...
    for size in [100, 1000] {
        ...
        for db_name in ["zerodb", "heed", "rocksdb", "redb"] {
            ...
            group.bench_with_input(BenchmarkId::new(db_name, size), &read_keys, |b, keys| {
                b.iter(|| {
...
fn bench_full_scan(c: &mut Criterion) {
    ...
    for size in [100, 1000] {
        ...
        for db_name in ["zerodb", "heed", "rocksdb", "redb"] {
            ...
            group.bench_with_input(BenchmarkId::new(db_name, size), &size, |b, _| {
                b.iter(|| {
