...
|-----------|----------------|----------|---------------|
| `EnvOpenOptions::read_txn_with_tls()` / `WithTls` TLS-bound read txns | `MDB_env` TLS slots | SHOULD — **DEFERRED** (compile-only shim; M2.7 sweep) | Every consumer opens `WithoutTls`. The `WithTls` marker, the builder method and the deref chain all exist and compile; what does **not** exist is real TLS-slot binding (a `WithTls` read txn is not actually thread-pinned). Deferred deliberately: this is ADR-0003 open question OQ5 — whether ZeroDB should *emulate* `MDB_TLS` slot reuse or declare `WithoutTls` the only real mode and document `WithTls` as an alias — which is a **design decision needing a maintainer**, not a missing implementation. Nothing in the consumer tree can observe the difference (`MDB_NOTLS` is universal, row 2). |
