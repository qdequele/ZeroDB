...
| 3 | `.map_size(usize)` | milli, ms, arroy(t), hannoy(t) | `mdb_env_set_mapsize` | Always clamped to page size (`clamp_to_page_size`) before the call. heed 0.22.1 does expose `unsafe fn Env::resize(usize)` (`envs/env.rs`, `mdb_env_set_mapsize` at runtime), but **no consumer calls it** (re-verified against Meilisearch v1.53.1, 2026-09-09: the only `.resize(` calls are on `Vec`s) — Meilisearch grows by reopening with a larger map after `MapFull` (row 55). The adapter therefore has no `resize`; calling one is a compile error, not a silent no-op (docs/COMPATIBILITY.md). [Corrected 2026-09-09: this row previously claimed heed had no `resize`.] | 1.2 |
| 4 | `.max_dbs(u32)` | milli(`NUMBER_OF_DBS`=27+geo), ms (auth=2, meilitool=100, versioning) | `mdb_env_set_maxdbs` | Named-DB catalog capacity. Bounded at open: values above `MAX_DBS_LIMIT` (2^20) are `Io(InvalidInput)` — the comparator registry allocates `max_dbs` slots eagerly (2026-09-09, security review M6). | 1.6 |
...
| 35 | `Database::put_reserved(txn, k, len, f)` (+ `ReservedSpace`) | milli (word-prefix docids) | `mdb_put` + `MDB_RESERVE` | Reserves `len` bytes in-place and writes the value directly into the map via the closure (avoids a temp buffer). Interacts with `WRITE_MAP` (row 8) and the dirty-page borrow model (SPEC 04). **B6 (2026-07-21):** the adapter fills the engine's in-frame slot directly (no heap detour); fork-pinned failing-closure semantics — the reserve happens *before* the closure, so a closure error propagates (`Io`) but **leaves the entry in place** with the reserved length (oracle pin `put_reserved_failing_closure_leaves_entry_parity`); zerodb zero-fills the unwritten tail (LMDB leaves whatever the page held — unobservable to the pin, byte-deterministic for us). | 1.10 |
...
|-----------|----------------|----------|---------------|
| `EnvOpenOptions::read_txn_with_tls()` / `WithTls` TLS-bound read txns | `MDB_env` TLS slots | SHOULD — **DEFERRED** (compile-only shim; left open by the M2.7 sweep, 2026-07-20) | Every consumer opens `WithoutTls`. The `WithTls` marker, the builder method and the deref chain all exist and compile; what does **not** exist is real TLS-slot binding (a `WithTls` read txn is not actually thread-pinned). Deferred deliberately: this is ADR-0003 open question OQ5 — whether ZeroDB should *emulate* `MDB_TLS` slot reuse or declare `WithoutTls` the only real mode and document `WithTls` as an alias — which is a **design decision needing a maintainer**, not a missing implementation. Nothing in the consumer tree can observe the difference (`MDB_NOTLS` is universal, row 2). |
...
| `Comparator` / `LexicographicComparator` / `IntegerComparator` / custom `mdb_set_compare` | `mdb_set_compare` / `mdb_set_dupsort` | **LANDED M2.4** (key); `mdb_set_dupsort` → 2.8 | No consumer sets a custom key comparator; all rely on `DefaultComparator` (row 53). **M2.4:** exposed as a **safe Rust trait** (`zerodb::Comparator`, object-safe, `Send + Sync`, plus `FnComparator` for the closure form) rather than LMDB's raw `MDB_cmp_func` pointer, registered per named database at open/create. Settable on **named DBs only** — the main/unnamed DB is the named-DB catalog and the GC tree is BE-txnid-keyed, so both stay memcmp unconditionally (SPEC 03 §2.0). **Defect found and fixed:** the adapter's `DatabaseOpenOptions::key_comparator::<C>()` type parameter was accepted and then *silently ignored* since M1.13 — every database was memcmp regardless. It now reaches the engine (`DefaultComparator` still short-circuits to the inlined memcmp path, so the consumer-facing default is bit-for-bit unchanged). **Hazard:** the comparator is not persisted (D-014). **Scope left open:** compacting copy refuses a custom-comparator env; `check`/`dump`/`load` stay memcmp-defined. The **dupsort** comparator is milestone 2.8, out of scope. Tests: `zerodb/tests/custom_comparator.rs` (13), `heed-zerodb/tests/phase2_extensions.rs`. |
...
| `env_closing_event(path)` free function | global registry lookup | **LANDED as a documented `None`** (M1.13; confirmed by the M2.7 sweep) | Consumers use `prepare_for_closing()` (row 23), which returns the event directly; the path-lookup form is unused. The function exists and always returns `None`: ZeroDB's env registry is keyed for the `EnvAlreadyOpened` check (TXN-51) and holds no path→closing-event map, and inventing one to serve an API no consumer calls would add a global lifetime-coupled structure for nothing. Unlike `clear_stale_readers` (M2.2), `None` here is a genuine "not provided", not a semantically-correct answer — noted so a future maintainer can tell the two no-ops apart. |
...
|-------------|----------------------|-----------|---------------|
| `zerodb::EnvOpenOptions::page_size(u32)` / `get_page_size()`; `heed_zerodb::EnvOpenOptions::page_size(u32)` | none — LMDB 0.9 derives `me_psize` from the OS and offers no selector | 2.6 | Selects the **database** page size at env **creation**: a power of two in `[MIN_PAGE_SIZE, MAX_PAGE_SIZE]` = `[4096, 65536]` (SPEC 02 §0), independent of the OS page size. **Adapter default (amended 2026-07-21, perf-parity spike):** when the option is omitted, `heed_zerodb::EnvOpenOptions::open` creates new stores at the **OS page size clamped to the window** — LMDB parity with the fork's `me_psize = me_os_psize` (capped 64 K), so a store created through the heed surface gets the same geometry the fork would give it (16 K on Apple Silicon, 4 K on x86_64 Linux). The **native** `zerodb::EnvOpenOptions` default stays `DEFAULT_PAGE_SIZE` = 4096 (engine-level determinism for tests/tools). Invalid values → `Io(InvalidInput)` at `open` (the taxonomy already used for D-006/D-010 open-time rejections). **Creation-only**: reopening an existing store ignores the request and adopts the persisted geometry (SPEC 02 §3.2), exactly as for `map_size` — there is deliberately no "wrong expectation" error. Read the effective value back via `Env::page_size()` / `EnvStat::page_size`. Tests: `zerodb/tests/page_size_selection.rs` (10 — validation, 4K/8K/16K/32K/64K round-trips with GC + `check_image`, the reopen contract, and the cross-page-size equivalence property), `heed-zerodb/tests/phase2_extensions.rs`. |
...
| `zerodb::Env::reader_list() -> Vec<ReaderEntry>`; `heed_zerodb::Env::reader_list()` | `mdb_reader_list` — which heed does not expose in any form | 2.2 | One entry per **occupied** reader slot, in slot order: `slot`, the pinned snapshot `txnid` (`None` while a reader is mid-pin, SPEC 04 §4.3), and `age` = published txnid − pinned txnid. Free slots are omitted exactly as `mdb_reader_list` skips `mr_pid == 0` slots, so presence *is* liveness and `max_readers() - reader_list().len()` is the free count. No pid/tid column: under D-001 every reader is in this process and `RoTxn` is `Send`, so the owning thread is not a stable property. **The result is a sample, not a linearizable snapshot** — the table is lock-free, entries can be stale on return, and nothing in the engine consults it (the GC gate uses TXN-20, whose staleness is provably one-directional). Ages are all computed against one sampled commit point and saturate at 0. |
| `zerodb::Comparator` / `FnComparator` / `DefaultComparator`; `Env::{create,open}_database_with_comparator`; `heed_zerodb::DatabaseOpenOptions::key_comparator::<C>()` (now honored) | `mdb_set_compare` — a raw C function pointer heed never calls | 2.4 | Safe per-named-database key ordering. See the second table's comparator row and **SPEC 03 §2.0** for the full contract: named DBs only, not persisted (D-014), compacting copy refuses, `check` stays byte-defined. |
| `zerodb::CopyToFile::copy_to_file_with_progress` / `CopyProgress`; `heed_zerodb::Env::copy_to_path_with_progress` | none — `mdb_env_copy2` reports nothing and heed's copy is an opaque blocking call | 2.3 | Opt-in progress on the M1.12 copy path. `done` is monotone and reaches `total`; `total` is **exact** for `CompactionOption::Disabled` (the snapshot's page count) and an **estimate** for `Enabled` (live source pages read — the pages *written* are unknowable until packing finishes). The existing no-callback `copy_to_file`/`copy_to_path` signatures are untouched (heed parity) and now delegate to the instrumented path with a no-op callback, so the two cannot drift; a byte-equality test pins that. **Every callback fires before any destination write**, so a panicking callback leaves the destination byte-for-byte untouched and the source env fully usable (no leaked reader slot). Tests: `zerodb/tests/copy_progress.rs` (7), `heed-zerodb/tests/phase2_extensions.rs`. |
...
- Second table: **12 SHOULD**, **9 WON'T** at M0.1 (21 rows total; a few rows
  carry a "WON'T Phase 1 / SHOULD Phase 2" dual note, counted by their Phase-1
  verdict). As of M2.7 (2026-07-20) most SHOULD rows read **LANDED**; three
  remain **DEFERRED** (`WithTls` real TLS binding, `set_flags`, `get_or_put*`)
  — the per-row status column is authoritative.
