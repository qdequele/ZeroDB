...
| 7 | `unsafe { options.open(path) }` | milli, ms, arroy(t), hannoy(t) | `mdb_env_create` + `mdb_env_open` | Directory env (`data.mdb`/`lock.mdb`); never `NO_SUB_DIR`. | 1.2 |
...
| `Env::force_sync()` | `mdb_env_sync(force)` | SHOULD (2.5) | Not called; consumers rely on sync-on-commit. Phase 2 explicit-sync API. |
| `Env::stat()` (env-level) / `EnvStat` | `mdb_stat` on main | SHOULD (2.1) | Only per-DB `stat()` (row 49) and `info().map_size` (row 20) are used. |
...
| `Env::max_readers()` getter, `Env::max_key_size()` | `mdb_env_get_maxreaders` / `mdb_env_get_maxkeysize` | SHOULD | `max_key_size` is assumed (row 57) but never queried; expose for completeness. |
