...
pub unsafe fn simd_binary_search_avx2(
    ...
    keys[left..right].binary_search_by(|k| comparator(k, target))
        .map(|i| left + i)
        .map_err(|i| left + i)
}
...
mod tests {
    ...
    fn test_simd_binary_search() {
        ...
        unsafe {
            ...
            assert_eq!(
                simd_binary_search_avx2(&keys, b"eee", |a, b| a.cmp(b)),
                Ok(4)
            );
            
            // Test not finding key
            assert_eq!(
                simd_binary_search_avx2(&keys, b"dde", |a, b| a.cmp(b)),
                Err(4)
            );
        }
    }
    ...
    fn test_simd_prefix_match() {
        let keys: Vec<&[u8]> = vec![
            b"hello", b"help", b"hero", b"world", b"worry"
        ];
