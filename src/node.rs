...
impl<'a> KeyValue<'a> {
    ...
    pub fn data(key: impl Into<Cow<'a, [u8]>>, data: impl Into<Cow<'a, [u8]>>) -> Self {
        Self { key: key.into(), value: NodeValue::Data(data.into()) }
    }

    /// Create a page reference key-value pair
    pub fn page_ref(key: impl Into<Cow<'a, [u8]>>, page: PageId) -> Self {
        Self { key: key.into(), value: NodeValue::PageRef(page) }
    }

    /// Convert to owned
    pub fn into_owned(self) -> KeyValue<'static> {
        KeyValue { key: Cow::Owned(self.key.into_owned()), value: self.value.into_owned() }
    }
    ...
    pub fn as_ref(&self) -> KeyValue<'_> {
        KeyValue {
            ...
            value: match &self.value {
                ...
                NodeValue::PageRef(id) => NodeValue::PageRef(*id),
                NodeValue::Overflow { page, size } => {
                    NodeValue::Overflow { page: *page, size: *size }
                }
                NodeValue::SubDb { root, entries } => {
                    NodeValue::SubDb { root: *root, entries: *entries }
                }
            },
        }
    }
...
impl NodeBuilder {
    ...
    pub fn new() -> Self {
        Self { flags: NodeFlags::empty(), key_size: 0, value_size: 0 }
    }
...
impl<'a> NodeIterator<'a> {
    ...
    pub fn new(page: &'a crate::page::Page) -> Self {
        Self { page, current: 0, count: page.header.num_keys as usize }
    }
}
...
mod tests {
    ...
    fn test_node_builder() {
        let header = NodeBuilder::new().key_size(10).value_size(100000).big_data().build();
