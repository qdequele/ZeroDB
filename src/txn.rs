...
impl<'env, M: mode::Mode> Transaction<'env, M> {
    ...
    pub fn commit(mut self) -> Result<()> {
        match self.mode_data {
            ...
            ModeData::Write { ref mut dirty, ref mut freelist, ref mut segregated_freelist, ref mut next_pgno, .. } => {
...
impl<'env> Transaction<'env, Write> {
    ...
    pub fn get_page_cow(&mut self, page_id: PageId) -> Result<(PageId, &mut Page)> {
        ...
        if let ModeData::Write { ref mut dirty, ref mut freelist, ref mut next_pgno, .. } = self.mode_data {
            ...
            let mut new_page = inner.page_pool.get(new_page_id, original_page.header.flags | PageFlags::DIRTY);
    ...
    pub fn free_page(&mut self, page_id: PageId) -> Result<()> {
        if let ModeData::Write { ref mut freelist, ref mut segregated_freelist, .. } = self.mode_data {
            if let Some(seg_list) = segregated_freelist {
