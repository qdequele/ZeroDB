...
impl HierarchicalBloomFilter {
    ...
    pub fn insert(&self, key: &[u8], leaf_page: PageId, branch_pages: &[PageId]) {
        ...
        for &branch_id in branch_pages {
            branches.entry(branch_id)
                .or_insert_with(|| BloomFilter::new(&self.config))
                .insert(key);
        }
        
        // Update leaf filter
        let mut leaves = self.leaves.write();
        leaves.entry(leaf_page)
            .or_insert_with(|| PageBloomFilter::new(leaf_page, 100))
            .insert(key);
    }
    ...
    pub fn stats(&self) -> BloomStats {
        ...
        BloomStats {
            root: root_stats,
            branch_filters: branch_count,
            leaf_filters: leaf_count,
        }
    }
}
...
mod simd_bloom {
    ...
    pub unsafe fn contains_batch_avx2(
        filter: &BloomFilter,
        keys: &[&[u8]],
    ) -> Vec<bool> {
