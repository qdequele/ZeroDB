...
impl CompressedBranchPage {
    ...
    pub fn init_root(
        ...
        unsafe {
            let entry_ptr =
                page.data.as_mut_ptr().add(CompressedBranchHeader::SIZE) as *mut CompressedEntry;
            *entry_ptr = entry;
        }
    ...
    pub fn find_child_compressed<C: Comparator>(page: &Page, search_key: &[u8]) -> Result<PageId> {
        if !page.header.flags.contains(PageFlags::BRANCH) {
            return Err(Error::InvalidOperation("Not a branch page"));
        }

        let header = unsafe { &*(page.data.as_ptr() as *const CompressedBranchHeader) };
    ...
    pub fn add_compressed_entry(
        ...
        let header = unsafe { &mut *(page.data.as_mut_ptr() as *mut CompressedBranchHeader) };
        ...
        // Write entry
        let entry = CompressedEntry { prefix_len, suffix_len, child, suffix_offset };

        unsafe {
            let entry_ptr = page.data.as_mut_ptr().add(
                CompressedBranchHeader::SIZE + header.num_entries as usize * CompressedEntry::SIZE,
